...
def compile_assembly_asset(asset: AssemblyAsset, output_dir: str | os.PathLike[str], *,
    ...
    Portrait Bundle input; the only differences are `original_rgba=None`
    (an Assembly Bundle has no donor photo to compare against -- the
    Assembly reader must not read one) and `draw_order=asset.draw_order`
    (Composer's authored paint order, not AutoRig's own canonical table).
    """
