...
def register_donor(base, donor):
    ...
    if donor.shape[2] == 3 or int((donor[:, :, 3] > 128).all()):
        ...
        from .donor_image import key_flat_background
...
def main(argv=None) -> int:
    """Attach an expression pack to a finished run, either way round:
      ...
      python -m portrait_autorig.expression <run> eye_closed=<png> mouth_open=<png>
      python -m portrait_autorig.expression <run> --from-run <donor run> eye_closed mouth_open
