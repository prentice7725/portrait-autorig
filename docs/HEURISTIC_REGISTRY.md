...
| chest deformation basis v3 | ACTIVE | `physics_distribution.version: 3` replaces the flat horizontal/vertical gain pair with six precomputed per-vertex basis fields (Carrier/Volume/Sag/Follow/Shear/Compression); v1/v2 manifests keep their exact existing mapping (P2.5 directive #5, #21) | `check_chest_basis_fields.mjs`, `check_chest_basis_reference_parity.mjs` |
| mass carrier basis semantics | ACTIVE | `carrierY = core·upperRelease` (near unit at the lobe centre, fading toward the upper attachment and outer rim); q drives this lobe-local mass translation, while lock-aware lobe weights still own neckline/center safety | `check_chest_basis_fields.mjs`, `check_chest_basis_motion_shape.mjs`, `check_body_kick_pipeline.mjs` |
| carrier gain | EXPERIMENTAL | `carrier_gain` defaults to 1.0 and is QA-adjustable; it is a local relative translation, never a uniform topwear/root displacement | `check_chest_basis_reference_parity.mjs` |
| volume basis semantics | ACTIVE | `volumeX = u·core·upperRelease`, `volumeY = v·core·upperRelease·0.30`; shape volume remains separate from the mass carrier | `check_chest_basis_fields.mjs` |
| sag basis semantics | ACTIVE | `sagX = u·lower·core·0.10`, `sagY = lower·core`; lower mass moves more than upper (directive #10) | `check_chest_basis_fields.mjs`, `check_chest_basis_motion_shape.mjs` |
| velocity follow semantics | ACTIVE | spring velocity is concentrated toward the lower lobe via a tangential component (`tx, ty`), magnitude-clamped to `max_follow_px`, never added uniformly to the whole field (directive #11-12) | `check_chest_basis_motion_shape.mjs` |
| body shear semantics | ACTIVE | shared body velocity is a small shape modifier only (`shear_gain_x_s`/`shear_gain_y_s`, clamped to `max_shear_px`), never a second root translation (directive #13, #60) | `check_chest_basis_reference_parity.mjs` |
| compression semantics | ACTIVE | a fixed middle vertical band (`smoothstep(-0.55,-0.10,v)·(1-smoothstep(0.35,0.85,v))`) compresses opposite the lower sag so the response does not read as simple inflation (directive #14) | `check_chest_basis_fields.mjs` |
| basis gains | EXPERIMENTAL | per-profile `volume_gain`/`sag_gain`/`follow_gain_s`/`shear_gain_x_s`/`shear_gain_y_s`/`compression_gain` (`soft_morph.SHAPE_PROFILE_CONFIG`); `soft`/`springy` use the directive's own exact figures, `firm_bounce` is a reasoned interpolation pending corpus tuning | `tests/test_soft_morph.py::BasisPhysicsDistributionTests` |
| upper anchor thresholds | EXPERIMENTAL | `upper_anchor_start`/`upper_anchor_end` default -0.75/-0.15 (lobe-local v) | `check_chest_basis_fields.mjs` |
