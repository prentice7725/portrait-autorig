...
| chest deformation basis v3 | ACTIVE | `physics_distribution.version: 3` replaces the flat horizontal/vertical gain pair with five precomputed per-vertex basis fields (Volume/Sag/Follow/Shear/Compression); v1/v2 manifests keep their exact existing mapping (P2.5 directive #5, #21) | `check_chest_basis_fields.mjs`, `check_chest_basis_reference_parity.mjs` |
| volume basis semantics | ACTIVE | `volumeX = u·core·upperRelease`, `volumeY = v·core·upperRelease·0.30`; center does not rigidly translate, upper attachment is damped by `upperRelease` | `check_chest_basis_fields.mjs` |
